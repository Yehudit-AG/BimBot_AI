...
class ArtifactService:
    ...
    def get_logic_c_pairs(self, db: Session, job_id: uuid.UUID) -> Optional[Dict[str, Any]]:
        """Get LOGIC C (filtered) wall pair candidates artifact for a job."""
        try:
            artifact = db.query(Artifact).filter(
                Artifact.job_id == job_id,
                Artifact.artifact_type == "logic_c_pairs"
            ).first()
            if not artifact:
                return None
            content = self.get_artifact_content(artifact)
            ...
            logging_service.logger.error(
                "Failed to get logic C pairs",
                job_id=str(job_id),
                error=str(e)
            )
            return None

    def get_logic_d_rectangles(self, db: Session, job_id: uuid.UUID) -> Optional[Dict[str, Any]]:
        """Get LOGIC D (containment-pruned) rectangles artifact for a job."""
        try:
            artifact = db.query(Artifact).filter(
                Artifact.job_id == job_id,
                Artifact.artifact_type == "logic_d_rectangles"
            ).first()
            if not artifact:
                return None
            content = self.get_artifact_content(artifact)
            ...
            logging_service.logger.error(
                "Failed to get logic D rectangles",
                job_id=str(job_id),
                error=str(e)
            )
            return None
    ...
    def get_door_rectangle_assignments(self, db: Session, job_id: uuid.UUID) -> Optional[Dict[str, Any]]:
        """Get door-rectangle assignment summary artifact for a job."""
        try:
            artifact = db.query(Artifact).filter(
                Artifact.job_id == job_id,
                Artifact.artifact_type == "door_rectangle_assignments"
            ).first()
            if not artifact:
                return None
            content = self.get_artifact_content(artifact)
            if not isinstance(content, dict):
                return None
            return {
                "door_assignments": content.get("door_assignments", []),
                "algorithm_config": content.get("algorithm_config", {}),
                "totals": content.get("totals", {}),
            }
        except Exception as e:
            logging_service.logger.error(
                "Failed to get door rectangle assignments",
                job_id=str(job_id),
                error=str(e)
            )
            return None
